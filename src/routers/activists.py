...
from database.models import (
    PostInGroup, PostInGroupType, Student, Person, 
    Group, StudentInGroup, Course, Curator
)
...
def index():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if action == 'save' and student_id and post_id:
                ...
                for cid in course_ids:
                    if cid.isdigit():
                        exists = db.query(PostInGroup).filter_by(
                            student_id=int(student_id),
                            post_in_group_type_id=int(post_id),
                            course_id=int(cid)
                        ).first()
        ...
        if selected_group_id:
            group_students = db.query(StudentInGroup).filter_by(group_id=int(selected_group_id)).all()
            student_ids = [s.student_id for s in group_students]
            
            if student_ids:
                records = db.query(PostInGroup).filter(PostInGroup.student_id.in_(student_ids)).all()
                for rec in records:
                    ...
                    if key not in activists_map:
                        ...
                        post_type = db.query(PostInGroupType).filter_by(post_in_group_type_id=rec.post_in_group_type_id).first()
                        activists_map[key] = {
                            'student_id': rec.student_id,
                            'post_id': rec.post_in_group_type_id,
                            'full_name': f"{person.surname} {person.name} {person.patronymic or ''}".strip() if person else "—",
                            'post_name': post_type.post_in_group_type_name if post_type else "—",
                            'courses': []
                        }
        ...
        if edit_student_id and edit_post_id:
            ...
            current_recs = db.query(PostInGroup).filter_by(
