...
from database.models import (
    StudentHobby, HobbyType, Student, Person, 
    Group, StudentInGroup, Curator
)
from utils.formatters import format_group_name

extracurricular_bp = Blueprint('extracurricular', __name__, url_prefix='/extracurricular')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if action == 'delete' and student_id:
                db.query(StudentHobby).filter_by(student_id=int(student_id)).delete(synchronize_session=False)
                db.commit()
                flash('Хобби удалены', 'success')
                return redirect(url_for('extracurricular.index', group_id=selected_group_id))
            
            if action == 'save' and student_id:
                db.query(StudentHobby).filter_by(student_id=int(student_id)).delete(synchronize_session=False)
                for hid in hobby_ids:
                    if hid.isdigit():
                        db.add(StudentHobby(student_id=int(student_id), hobby_type_id=int(hid)))
                db.commit()
        ...
        if selected_group_id:
            sig_records = db.query(StudentInGroup).filter_by(group_id=int(selected_group_id)).all()
            student_ids = [s.student_id for s in sig_records]
            
            if student_ids:
                records = db.query(StudentHobby).filter(StudentHobby.student_id.in_(student_ids)).all()
                for rec in records:
                    if rec.student_id not in students_hobbies:
                        person = db.query(Person).filter_by(person_id=rec.student_id).first()
                        students_hobbies[rec.student_id] = {
                            'person_id': rec.student_id,
                            'full_name': f"{person.surname} {person.name} {person.patronymic or ''}".strip() if person else "—",
                            'hobbies': []
                        }
                    hobby = db.query(HobbyType).filter_by(hobby_type_id=rec.hobby_type_id).first()
                    if hobby:
                        students_hobbies[rec.student_id]['hobbies'].append(hobby.hobby_type_name)
        ...
        if edit_student_id:
            ...
            assigned = db.query(StudentHobby).filter_by(student_id=int(edit_student_id)).all()
