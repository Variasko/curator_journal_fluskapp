...
def format_group_name(group):
    ...
    qual = group.qualification.qualification_reduction if group.qualification else ""
    ...
    try:
        if request.method == "POST":
            ...
            if action in ("add", "edit"):
                ...
                if not all([spec_id, course_id, creation_year]):
                ...
                qual_id_val = int(qual_id) if qual_id else None
                curator_id_val = int(curator_id) if curator_id else None

                if action == "edit":
                    group_id = int(request.form.get("group_id"))
                    group = db.query(Group).get(group_id)
                    if group:
                        group.specialization_id = spec_id
                        group.qualification_id = qual_id_val
                        group.course_id = course_id
                        group.creation_year = creation_year
                        group.curator_id = curator_id_val
                else:
                    new_group = Group(
                        specialization_id=spec_id,
                        qualification_id=qual_id_val,
                        course_id=course_id,
                        creation_year=creation_year,
                        curator_id=curator_id_val,
                    )
                    db.add(new_group)
        ...
        groups_data = [
            {"group": g, "display_name": format_group_name(g)} for g in groups_raw
        ]
