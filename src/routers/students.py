...
def manage_students():
    ...
    try:
        if request.method == 'POST':
            ...
            if action in ('add', 'edit'):
                ...
                if action == 'edit':
                    ...
                    existing_person = db.query(Person).filter(Person.phone == phone, Person.phone.isnot(None)).first()
                    if existing_person:
                    ...
                    existing_student = db.query(Student).filter(Student.person_id == person_id).first()
                ...
                if group_id:
                    gid = int(group_id)
                    sig = db.query(StudentInGroup).filter(StudentInGroup.student_id == person_id).first()
                    if sig:
                        sig.group_id = gid
                    else:
                        db.add(StudentInGroup(student_id=person_id, group_id=gid, creation_date=date.today()))

                db.commit()
                flash('Данные сохранены' if action == 'add' else 'Данные обновлены', 'success')
                return redirect(url_for('students.manage_students'))
        ...
        for student, person in students:
            ...
            sig = db.query(StudentInGroup).filter(StudentInGroup.student_id == student.person_id).first()
        ...
        edit_group_id = None
        if edit_id:
            edit_student = db.query(Student).filter(Student.person_id == int(edit_id)).first()
            if edit_student:
                edit_person = edit_student.person
                sig = db.query(StudentInGroup).filter(StudentInGroup.student_id == int(edit_id)).first()
