...
from database.models import ClassHour, VisitingClassHour, Student, Person, Group, StudentInGroup
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if action == 'delete' and ch_id:
                ...
                db.query(VisitingClassHour).filter_by(class_hour_id=int(ch_id)).delete(synchronize_session=False)
                db.query(ClassHour).filter_by(class_hour_id=int(ch_id)).delete(synchronize_session=False)
                db.commit()
                flash('Запись удалена', 'success')
                return redirect(url_for('class_hours.index', group_id=selected_group_id))
            ...
            if action in ('add', 'edit'):
                ...
                sigs = db.query(StudentInGroup).filter_by(group_id=int(selected_group_id)).all()
                ...
                if action == 'edit':
                    ...
                    if ch:
                        ...
                        db.query(VisitingClassHour).filter_by(class_hour_id=ch.class_hour_id).delete(synchronize_session=False)
                else:
                    ch = ClassHour(class_hour_date=ch_date, group_id=int(selected_group_id))
                    db.add(ch)
                    db.flush()  
                ...
                for sid in all_student_ids:
                    ...
                    db.add(VisitingClassHour(class_hour_id=ch.class_hour_id, student_id=sid, is_visited=is_visited))
        ...
        if selected_group_id:
            raw_ch = db.query(ClassHour).filter_by(group_id=int(selected_group_id)).all()
            for ch in raw_ch:
                count = db.query(VisitingClassHour).filter_by(class_hour_id=ch.class_hour_id, is_visited=True).count()
                class_hours_list.append({
                    'id': ch.class_hour_id,
                    'date': ch.class_hour_date.strftime('%d.%m.%Y') if ch.class_hour_date else '—',
        ...
        if edit_id:
            ...
            if edit_ch:
                visits = db.query(VisitingClassHour).filter_by(class_hour_id=edit_ch.class_hour_id, is_visited=True).all()
