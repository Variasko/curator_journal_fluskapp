    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if action == 'delete' and record_id:
                db.query(ObservationList).filter_by(observation_list_id=int(record_id)).delete()
                db.commit()
                flash('Запись удалена', 'success')
                return redirect(url_for('observation.index', group_id=selected_group_id))
        ...
        if selected_group_id:
            ...
            if student_ids:
                recs = db.query(ObservationList).filter(ObservationList.student_id.in_(student_ids)).all()
                for rec in recs:
                    person = db.query(Person).filter_by(person_id=rec.student_id).first()
                    observations.append({
                        'id': rec.observation_list_id,
                        'student_id': rec.student_id,
                        'student_name': f"{person.surname} {person.name} {person.patronymic or ''}".strip() if person else "—",
                        'date': rec.observation_date.strftime('%d.%m.%Y') if rec.observation_date else "—",
