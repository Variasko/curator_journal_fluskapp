    ...
    try:
        if request.method == 'POST':
            ...
            if action == 'delete':
                ...
                if role: db.delete(role); db.commit()
            ...
            if action in ('add', 'edit'):
                ...
                if action == 'edit':
                    role = db.query(Role).get(int(request.form.get('role_id')))
                    if role: role.role_name, role.role_description = name, desc or None
                else:
        ...
        return render_template('admin_roles.html', title="Роли", items=items, edit_item=edit_item)
    finally:
        db.close()