...
from database.models import (
    SocialPassport, Student, Person, SocialStatus, 
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if action == 'save' and student_id:
                ...
                for sid in status_ids:
                    if sid.isdigit():
                        db.add(SocialPassport(student_id=int(student_id), social_status_id=int(sid)))
                db.commit()
        ...
        if selected_group_id:
            ...
            for sig in sig_records:
                student = db.query(Student).join(Person).filter(Student.person_id == sig.student_id).first()
                if student and not student.is_expelled:  
                    
                    assigned = db.query(SocialPassport, SocialStatus)\
                        .join(SocialStatus)\
        ...
        edit_assigned_ids = []
        if edit_student_id:
            student = db.query(Student).join(Person).filter(Student.person_id == int(edit_student_id)).first()
            if student:
                edit_student_name = f"{student.person.surname} {student.person.name} {student.person.patronymic or ''}".strip()
                assigned = db.query(SocialPassport).filter_by(student_id=int(edit_student_id)).all()
