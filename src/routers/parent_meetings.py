...
parent_meetings_bp = Blueprint('parent_meetings', __name__, url_prefix='/parent_meetings')
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if action == 'delete' and meeting_id:
                db.query(ParentMeeting).filter_by(parent_meeting_id=int(meeting_id)).delete()
                db.commit()
                flash('Протокол удалён', 'success')
                return redirect(url_for('parent_meetings.index', group_id=selected_group_id))
        ...
        if selected_group_id:
            raw_meetings = db.query(ParentMeeting).filter_by(group_id=int(selected_group_id)).all()
            for m in raw_meetings:
                meetings.append({
                    'id': m.parent_meeting_id,
                    'date': m.meeting_date.strftime('%d.%m.%Y') if m.meeting_date else '—',
                    'topic': m.topics or '—'
                })
