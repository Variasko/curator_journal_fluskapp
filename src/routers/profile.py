...
from database.models import Person, Curator, Group, StudentInGroup, ObservationList, ClassHour
import io
...
def index():
    ...
    try:
        ...
        student_ids_in_groups = db.query(StudentInGroup.student_id)\
            .filter(StudentInGroup.group_id.in_(group_ids)).all()
        student_ids_list = [s[0] for s in student_ids_in_groups]
        
        stats = {
            'groups': len(groups),
            'students': len(student_ids_list),
            'observations': db.query(ObservationList).filter(ObservationList.student_id.in_(student_ids_list)).count() if student_ids_list else 0,
            'class_hours': db.query(ClassHour).filter(ClassHour.group_id.in_(group_ids)).count() if group_ids else 0
        }
